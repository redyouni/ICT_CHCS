...
public class Utility {
	...
	public static void screenshot(Context context, View view)throws Exception {      
		...
		try {
			...
			Toast.makeText(context, "���옣�릺�뿀�뒿�땲�떎.", Toast.LENGTH_SHORT).show();
	...
	/*public static void setVersion(Context context) {
		try {
			Log.d("Application", "versionName : " + context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionName);
			Log.d("Application", "getEXTRA_VERSION() : " + context.getSharedPrefs_VERSION());

			if(getSharedPrefs_VERSION().equalsIgnoreCase(context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionName) == false) {
				setSharedPrefs_VERSION(context.getPackageManager().getPackageInfo(context.getPackageName(), 0).versionName);
			}
	...
	public static ArrayList<String> CheckUsbExist() {		
		...
		if (files != null) 	{
			for (File file : files)	 {
				if (file.getPath().startsWith(storage +"/sd") /*&& !file.getPath().startsWith(storage+"/sdcard")*/)	{  //usb & sdcard
					if(file.canRead())	{
	...
	public static boolean isOnline(Context context) { // network �뿰寃� �긽�깭 �솗�씤
		try {
	 ...
	 * @param interfacename eth0, ipv4  true=return ipv4, false=return ipv6
